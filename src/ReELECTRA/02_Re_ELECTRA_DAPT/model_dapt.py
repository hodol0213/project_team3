...
from transformers import (
    Trainer, TrainerCallback, TrainingArguments, TrainerState, TrainerControl,
...
class ElectraCheckpointCallback(TrainerCallback):
    """
    Save generator+discriminator state_dict when Trainer triggers save.
    Set cb.trainer_ref = trainer after trainer creation.
    """
    def __init__(self):
...
class ElectraTrainer(Trainer):
    ...
    def training_step(self, model, inputs, loss_or_steps=None, **kwargs):
        ...
        # Gradient clipping
        torch.nn.utils.clip_grad_norm_(list(generator.parameters()) + list(discriminator.parameters()), max_norm=self.args.max_grad_norm)
